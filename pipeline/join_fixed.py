"""
Segment Merging / Join Step (Step 3 of the pipeline — fixed version)

This is the corrected version of join.py.  The original used a for-loop
and attempted to reassign the loop variable after merging segments, which
Python's for-loop ignores.  This version uses a while-loop so that after
a merge the index properly resets and the newly-combined segment can be
re-compared with the next one.

See join.py module docstring for a full description of the algorithm.

"""
Label Typo Fixer (Step 1b of the pipeline — optional pre-processing)

Scans every CSV in to-label/ and auto-corrects common data-entry errors
that would cause validate_input.py or process_data.py to fail.

Fixes applied (in order):
    1. Trailing empty columns — strips columns beyond the expected 4.
    2. Header typos — "ambigious", "ambigous", "ambiguos", "Start"/"End",
       "Trancript", "Transicript".
    3. Alternate column orderings:
       - 4-column [in/out, Transcript, Final_Beginning, Final_End]
         → reordered to [in/out, start, end, Transcript]
       - 8-column format with JAS/JOC/Final columns
         → reduced to 4 columns using the Final_* columns.
    4. Data-row typos in the in/out/ambiguous column (e.g. "oin" → "in",
       "pout" → "out", empty → "out").
    5. Data-row typos in start/end columns (e.g. "RUE" → "TRUE",
       "TRUETRUE" → "TRUE", empty → "FALSE").

Files are modified in-place.  Run validate_input.py afterward to confirm
...
def fix_anamt3_columns(filepath):
    """Special-case fix for anamt3.csv which has swapped column order.

    Currently commented out in fix_file() — uncomment if processing the
    siblings dataset and anamt3.csv still has its original column layout.
    """
...
def fix_file(filepath):
    """Apply all typo/format fixes to a single CSV file (in-place).
